...
export default function SignIn() {
  ...
  return (
    <section className="mx-auto max-w-md px-6 py-24">
      <h1 className="text-3xl font-black text-paper">{t.auth.loginTitle}</h1>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        >
          {loading ? t.auth.loading : t.auth.loginCta}
        </button>
      </form>

      <p className="mt-6 text-center text-sm text-paper-muted">
        {t.auth.noAccount}{" "}
        <Link to="/signup" className="text-ember font-bold hover:underline">
          {t.auth.signUp}
        </Link>
      </p>
    </section>
